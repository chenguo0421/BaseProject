...
class GiftItemAdapter(
    ...
    override fun onBindViewHolder(holder: MyHolder, position: Int) {
        ...
        Glide.with(context).load(data[position].url?.get(0)).into(holder.itemView.iv_content)
        holder.itemView.tv_name.text = data[position].name ?: ""
        holder.itemView.tv_price.text = data[position].price ?: ""
        holder.itemView.tv_num_convert.text = String.format(context.resources.getString(R.string.portal_store_item_convertnum) ,
            data[position].convertNum
        )
    }
