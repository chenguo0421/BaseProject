...
import com.cg.base.utils.DeviceUtils
...
class StoreProductAdapter(var context: Context, var data:ArrayList<StoreProductsBean>) : RecyclerView.Adapter<StoreProductAdapter.MyHolder>(){
    ...
    private fun setRecommendBoutiqueItem(holder: HolderForRecommendBoutique, position: Int) {
    ...
    private fun setChoicenessItem(holder: HolderForChoiceness, position: Int) {
        holder.itemView.tv_title.text = data[position].title?.get(0)?.title
        holder.itemView.tv_title.setTextColor(Color.parseColor(data[position].title?.get(0)?.titleColor))
    ...
    private fun setRecommendTodayItem(holder: HolderForRecommendToday, position: Int) {
    ...
    private fun setNewcomeItem(holder: HolderForNewcome, position: Int) {
