        ...
        for line in lines:
            ...
            if not re.match('^//',line.lstrip()):
               # print("line = %s" % line.lstrip())
                if re.match('@ApplicationDelegate',line.lstrip()):
                    # print("匹配到@ApplicationDelegate")
                    r = re.findall('@ApplicationDelegate\((.*?)\)',line.lstrip())
                    for each in r:
                        if re.search('=',each):
...
# 扫描每一个classpath，对每个java文件进行匹配，看有无指定注解，若有，返回该类的路径
def get_annotation_path_list1(list):
    _annotation_path_list = []
    for item in list:
        _fileList = scan_file_by_suffix(item,[".java",".kt"])
        for f in _fileList:
            bean = regLineForCRouter(f)
            if not "" == bean.clsPath:
                _annotation_path_list.append(bean)
    return _annotation_path_list


# 根据_annotationPathList生成ApplicationDelegates.xml文件
def write(xmlPath,list):
    print("ApplicationDelegate.xml文件路径为：[{0}]".format(xmlPath))
    if os.path.exists(xmlPath) and os.path.isfile(xmlPath):
        os.remove(xmlPath)
    # 在内存中创建一个空的文档
    doc = Dom.Document()
    # 创建一个根节点Managers对象
    root = doc.createElement('ApplicationDelegates')
    # 将根节点添加到文档对象中
    doc.appendChild(root)
    for bean in list:
        nodeCRouter = doc.createElement('ApplicationDelegate')
        # 由于一个类只有一个路由，这里直接取第0个
        if len(bean.clsAnnotation) > 0:
            nodeRouterPath = doc.createElement('ModuleName')
...
#生成ApplicationDelegate.xml
def build_delegate_xml():
    if _annotationBeanList is None or len(_annotationBeanList) <= 0:
        return
    try:
        ...
        xmlPath = os.path.join(assetsPath,"ApplicationDelegate.xml")
