        ...
        for line in lines:
            ...
            if not re.match('^//',line.lstrip()):
               # print("line = %s" % line.lstrip())
                if re.match('@JNPlugin',line.lstrip()):
                    # print("匹配到@ApplicationDelegate")
                    r = re.findall('@JNPlugin\((.*?)\)',line.lstrip())
                    for each in r:
                        if re.search('=',each):
...
# 根据_annotationPathList生成JNPlugins.xml文件
def write(xmlPath,list):
    print("JNPlugins.xml文件路径为：[{0}]".format(xmlPath))
    if os.path.exists(xmlPath) and os.path.isfile(xmlPath):
        os.remove(xmlPath)
    # 在内存中创建一个空的文档
    doc = Dom.Document()
    # 创建一个根节点Managers对象
    root = doc.createElement('JNPlugins')
    # 将根节点添加到文档对象中
    doc.appendChild(root)
    for bean in list:
        nodeCRouter = doc.createElement('JNPlugin')
        # 由于一个类只有一个路由，这里直接取第0个
        if len(bean.clsAnnotation) > 0:
            nodeRouterPath = doc.createElement('plugin-name')
        ...
        nodeClassPath = doc.createElement("plugin-path")
...
def build_delegate_xml():
    ...
    try:
        ...
        xmlPath = os.path.join(assetsPath,"JNPlugins.xml")
